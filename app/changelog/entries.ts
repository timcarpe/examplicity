...
export const changelogEntries: readonly ChangelogEntry[] = [
  {
    ...
    publicChanges: [
      'Updated Binary Numbers, Coordinate Geometry, Graph Search, Prime Factors, Sequences and Histograms with clearer working areas and consistent progress feedback. Added consistent activity top panels with clearer task headings, clearer primary, secondary and disabled actions, and an algorithm toggle for Graph Search that starts ready in Dijkstra mode, lightweight action and calculation cards, consistently ordered, rounded grey Working and blue exam toggles with subtle container shadows and regular-weight labels, movable completion cards and improved chart dragging and readability. Aligned Sequence’s grid and pattern squares across responsive sizes. Clarified Sequence’s previous-stage container, descriptor spacing and contextual return cue, with rounded containers, a gentle introduction highlight and compact violet-tinted backgrounds for displayed mathematics and working-card pulses that stop on interaction. Removed duplicate corner status marks from cards with answer fields.',
      'Refreshed Repeated Percentage Change, Motion Graphs, Bounds, Straight Lines and Recurring Decimals with consistent activity controls, readable working cards and movable completion prompts. Motion Graphs now exposes Working settings for each graph mode. Percentage changes can be applied by dragging the bar itself. Bounds restores rounding-boundary discovery, draggable speed exploration and finer-precision comparison alongside right-hand working and predictions. Motion Graphs has fewer overlapping labels, and Recurring Decimals uses clearer explanations, approved concept colours and direct interaction cues. Charts fit their containers without internal scrolling, card titles have a clearer regular-weight hierarchy, and field focus uses a darker border without an extra outline.',
    ...
    internalChanges: [
      ...
      'Published the living design guide and Lab Kit resources under /developer for the next release, with focused component-use rules and interactive specimens. Extended working attention and flat maths surfaces across the reviewed labs. Consolidated refinement CSS into a shared authoring-kit resource embedded into published and downloaded labs. Recorded the six-lab design changes and working-progression adaptations for later review; validation of the latest five-lab working-area batch was skipped at the user’s request.',
