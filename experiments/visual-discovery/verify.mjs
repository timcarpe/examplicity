...
try{
 ...
 if(!focus||focus==='maths'){
  ...
  await named('Test the model');await expectReady(true);
  await page.locator('#scaleAnswer').fill('2');await expectReady(false);
  await page.locator('#scaleAnswer').fill('3');await named('Test the model');await next();
  assert.equal((await value()).step,4);
  await keys('[data-control=scale]','Home');await keys('[data-control=scale]','ArrowRight',15);
  await page.locator('#scaleAnswer').fill('4');await expectReady(false);await named('Test the model');await expectReady(false);
  await click('#evidenceHelp summary');await page.waitForFunction('$("hintText").textContent.length > 0');assert.match(await page.locator('#hintText').innerText(),/edge length/);
  await click('#anotherHint');assert.match(await page.locator('#hintText').innerText(),/three-direction/);
  await click('#evidenceHelp summary');
  await page.locator('#scaleAnswer').fill('2');await expectReady(false);await named('Test the model');await expectReady(true);
  ...
  await page.locator('#scaleAnswer').fill('2');await keys('[data-control=scale]','Home');await keys('[data-control=scale]','ArrowRight',15);await named('Test the model');
 ...
 if(!focus||focus==='physics'){
  ...
  await named('Mark this angle');assert.ok(Math.abs(await read('s.angle')-41.8103148958)<.001);await next();
  await named('Replace glass with diamond');await page.locator('#ratioInput').fill('0.5');await page.locator('#angleInput').fill('45');await named('Test the beam');await expectReady(false);
  await page.locator('#ratioInput').fill('0.413');await page.locator('#angleInput').fill('24.4');await expectReady(false);await named('Test the beam');await expectReady(true);
  await page.locator('#ratioInput').fill('0.4');await expectReady(false);await page.locator('#ratioInput').fill('0.413');await named('Test the beam');await next();
  assert.equal(await read('s.inside'),'water');assert.equal(await page.locator('#ratioInput').count(),0);
  await page.locator('#angleInput').fill('42');await named('Test the beam');await expectReady(false);
  await page.locator('#angleInput').fill('48.8');assert.equal(await page.locator('#angleInput').getAttribute('class'),'');
  await named('Test the beam');await expectReady(false);assert.equal(await page.getByRole('button',{name:'Reverse this boundary',exact:true}).count(),1);
 ...
 if(!focus||focus==='chemistry'){
  ...
  await page.locator('#pressureInput').fill('100');await named('Test expansion to 4 L');await expectReady(false);assert.equal(await read('s.V'),4);
  ...
  await named('Test expansion to 4 L');await expectReady(true);await next();
  assert.ok(!(await page.locator('#working').innerText()).includes('÷'));
  await page.locator('#productInput').fill('400');await page.locator('#volumeInput').fill('4');await named('Test predicted volume');await expectReady(false);
  await page.locator('#productInput').fill('600');await page.locator('#volumeInput').fill('2.5');await expectReady(false);await named('Test predicted volume');await expectReady(true);
 ...
 if(!focus||focus==='physics'){
  ...
  await keys('[data-control=angle]','End');await keys('[data-control=angle]','Home');await keys('[data-control=angle]','ArrowRight',37);await named('Mark this angle');
