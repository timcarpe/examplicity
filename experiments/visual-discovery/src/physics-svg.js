function sceneHeight(){return mobile?430:420}
function opticsLayout(){return{cx:W*.5,cy:mobile?178:180,r:mobile?106:162}}
function ray(x1,y1,x2,y2,colour,width,opacity=1){
 const g=el('g',{opacity});line(x1,y1,x2,y2,{stroke:colour,'stroke-width':width,'stroke-linecap':'round'},g);
...
function render(){
 ...
 const {cx,cy,r}=opticsLayout(),radius=r+24,c=critical(),o=optics(),predict=step===2&&!s.tested,a=materials[s.inside],b=materials[s.outside];
 ...
 el('rect',{x:edgeLeft,y:25,width:edgeRight-edgeLeft,height:cy-25,fill:'var(--lab-surface-muted)'});
 const defs=el('defs'),glass=el('linearGradient',{id:'blockMaterial',x1:'0%',y1:'0%',x2:'0%',y2:'100%'},undefined,defs);
 el('stop',{offset:0,'stop-color':'var(--lab-concept-blue-soft)'},undefined,glass);el('stop',{offset:1,'stop-color':'var(--lab-concept-blue-fill)'},undefined,glass);
 el('path',{d:`M ${cx-r} ${cy} A ${r} ${r} 0 0 0 ${cx+r} ${cy} Z`,fill:s.inside==='air'?'var(--lab-surface-muted)':'url(#blockMaterial)',stroke:'var(--lab-concept-blue-line)','stroke-width':2});
 ...
 text(cx,H-23,`${a.name} · n = ${fmt(a.n,2)}`,'small','middle');
