...
function go(n){step=clamp(n,0,lesson.length-1);maxStep=Math.max(maxStep,step);$('controls').replaceChildren();$('context').replaceChildren();$('working').hidden=true;$('records').innerHTML='';$('records').hidden=true;ready=false;enter();resize();$('title').focus({preventScroll:true})}
...
function workSetup(title,body,note='',required=false){
 ...
 area.innerHTML=`<h2>${title}</h2><div class="work-steps">${body}</div><p class="work-note" id="workNote">${note}</p><div class="work-actions" id="workActions"></div>`;
 area.oninput=area.onpointerdown=()=>{area.dataset.interacted='true'};
}
function workValue(id,value){const node=$(id);if(!node||node.textContent===String(value))return;const changed=node.textContent!=='';node.textContent=value;if(changed&&!reduced.matches)node.animate([{backgroundColor:getComputedStyle(document.documentElement).getPropertyValue('--lab-needed-soft')},{backgroundColor:getComputedStyle(node).backgroundColor}],{duration:650})}
