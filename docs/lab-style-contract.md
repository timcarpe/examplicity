...
| `packages/lab-kit/src/lab-design-purge.json` | Remix primitives retained by selective CSS packaging |
...
Available unattempted cards pulse their needed border until interaction with their input or associated model. Remember dismissal through redraws; restart it for a fresh activity. Use the blue cue’s 2px outline and 4–9px halo for yellow working attention. Red uses the same pulse for a current correction: set `data-work-state="bad"` and `data-lab-attention="true"` on a `lab-work-card`, and dismiss with `data-interacted="true"` on interaction. A red card targeted by `LabDesign.attention` also uses red; clear that target when the required step changes. Locked, reference, completed and non-actionable mistake cards stay static. Reduced motion uses a static border.
