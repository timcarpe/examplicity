...
### Review recovery: recurring decimals, time zones and shared controls

- Shared Working and Exam controls now follow the Critical Angle rounded-rectangle reference: 10px track / 7px segment corners, regular 12px text, selected charcoal, centered 11px labels above with space. Existing options, setting order and availability remain.
- Recurring Decimals: removed redundant top route cards; centered the subtraction minus and fitted its full travel within the visual; thin approved tail colours; input-level feedback; standard completion buttons; sequential long-division table with matching remainders and replay, plus optional denominator reasoning.
- Time Zones: fixed a double-grid composition that reserved an empty right column. Calendar/working columns now fill the workspace; meeting token sits beside the organiser area, below the goal. Existing None / Some / All calculation and proposal gates retained.
- Explicit reset destinations added in 23 existing labs: Parity, Transmission, Encryption, DNS, Graph Search, Prime Factors, Recurring Decimals, Bounds, Repeated Percentage, Sequence, Motion, Coordinate, Circle, Solids, Right Triangle, Non-right Triangle, 3D Trigonometry, Transformations, Vectors, Histogram, Relative Frequency, Replacement and Similarity. Buttons retain their original handlers. Single resets sit in model headings/corners; shared tool rows use spacing and rounded geometry without a box for one button.
- Canonical style contract and HTML examples updated; shared resources compiled into downloadable labs and the developer reference. Review scope includes all moved resets, both setting examples, recurring completion/incorrect fields/remainder replay and the three Time Zones working modes.
- Verification: 57-lab settings scan (38 groups) passed; all 23 relocated reset nodes remain visible and callable after reset. Recurring direct subtraction drag and endpoint fit passed at 1422px and 390px; None/Some/All, incorrect fields, recurring/terminating traces passed. Time Zones placement, all working levels, wrong/correct fields and send gates passed. Standalone/publication and contract checks passed for all 57 labs.
