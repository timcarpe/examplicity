...
- [ ] Preserve the approved authoring document at
  `labs-src/<subject>/<slug>/lab.html`, author its contract at
  `lab-contracts/<subject>/<slug>.lab.json`, and register it in `labs-src/manifest.json`.
...
cards link to the canonical `/labs/<subject>/<slug>` route, which server-renders
the existing iframe shell. The iframe loads the generated static document from
`/labs/<subject>/<slug>.html?embed=1`; that implementation URL is non-indexable.
The older `/computer-science/<exam>?lab=<slug>` form remains supported for
backward compatibility but is not generated by the catalogue.
...
Both the legacy flat Computer Science URLs and direct requests for the generated
HTML documents are permanent redirects generated from the manifest in
`next.config.ts`. New labs publish only one canonical shell route and one
embed-only static document; do not add a second copy of the HTML document.
...
- [ ] `slug` is unique, lowercase ASCII, and kebab-case. It is the basename of
  the file and the canonical route segment. `subject` is the stable subject
  directory ID. These values must agree exactly across the manifest,
  `public/labs/<subject>/<slug>.html`, and `/labs/<subject>/<slug>`. Do not
...
- [ ] With `npm run dev`, test the selected exam tabs and confirm the new card
  ...
  `/labs/<subject>/<slug>`, the loading screen resolves, and the iframe loads
  `/labs/<subject>/<slug>.html?embed=1`. Confirm a direct request for the raw
  `.html` URL redirects to the canonical shell route.
