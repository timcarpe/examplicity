(() => {
  ...
  function movableCompletion(target, boundsTarget, options = {}) {
    const card = required(target, options.root);
    const bounds = required(boundsTarget, options.root);
    const handle = options.handle ? required(options.handle, card) : card.querySelector?.("[data-dialog-drag], .dialog-move");
    if (!handle) throw new Error("LabKit direct.movableCompletion needs a move handle inside the completion card.");
    const inset = Math.max(0, Number(options.inset ?? 8) || 0);
    let x = 0;
    let y = 0;
    let drag = null;
    let moved = false;

    const boundedDelta = (delta, minimum, maximum) => minimum > maximum
      ? (minimum + maximum) / 2
      : Math.max(minimum, Math.min(maximum, delta));
    const move = (dx, dy) => {
      if (card.hidden) return;
      const cardRect = card.getBoundingClientRect();
      const boundsRect = bounds.getBoundingClientRect();
      x += boundedDelta(dx, boundsRect.left + inset - cardRect.left, boundsRect.right - inset - cardRect.right);
      y += boundedDelta(dy, boundsRect.top + inset - cardRect.top, boundsRect.bottom - inset - cardRect.bottom);
    ...
    const onClick = event => {
      ...
      const cardRect = card.getBoundingClientRect();
      const boundsRect = bounds.getBoundingClientRect();
      move(cardRect.left + cardRect.width / 2 < boundsRect.left + boundsRect.width / 2 ? boundsRect.right - inset - cardRect.right : boundsRect.left + inset - cardRect.left, 0);
    };
    const onKeyDown = event => {
    ...
    const resize = host.ResizeObserver ? new host.ResizeObserver(() => move(0, 0)) : null;
  ...
  const direct = Object.freeze({ keyboardAdjustable: bindKeyboardAdjustable, movableCompletion });
